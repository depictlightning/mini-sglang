...
auto store_cache_dispatch(at::Tensor k_cache, at::Tensor v_cache,
  ...
  TORCH_CHECK(k_cache.is_cuda() && v_cache.is_cuda() && out_loc.is_cuda() &&
                  k.is_cuda() && v.is_cuda(),
              "All tensors must be CUDA tensors");
  TORCH_CHECK(k_cache.sizes() == v_cache.sizes(),
              "k_cache and v_cache must have the same size");
  TORCH_CHECK(k_cache.strides() == v_cache.strides(),
              "k_cache and v_cache must have the same strides");
  ...
  TORCH_CHECK(k.stride(-1) == 1 && k_cache.stride(-1) == 1,
              "k and k_cache must be contiguous in head.");
  TORCH_CHECK(k.size(-1) == k_cache.size(-1),
              "k and k_cache must have the same head size");
  TORCH_CHECK(out_loc.dim() == 1 && out_loc.is_contiguous(),
              "out_loc must be a 1D contiguous tensor");
  static_assert(sizeof(uint64_t) == 8,
                "uint64_t must be 8 bytes, our code assumes that");
    ...
    if constexpr (!std::is_same_v<scalar_t, int32_t> &&
      ...
      if (size_bytes % 256 == 0) {
        ...
        store_kv_cache_256x1<<<num_blocks, num_threads, 0, stream>>>(
            ...
            k_ptr, v_ptr, kv_cache_stride, kv_input_stride, items_per_warp);
