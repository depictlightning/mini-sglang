...
struct HiCacheKernel {
  static void run_one(
    ...
    const auto kernel =
        use_int32 ? hicache_transfer_per_layer<int32_t, kElementSize, kBlockQuota, kNumThreads, kMaxOccupancy, kUsePDL>
                  : hicache_transfer_per_layer<int64_t, kElementSize, kBlockQuota, kNumThreads, kMaxOccupancy, kUsePDL>;
  ...
  static void run_all(
    ...
    const auto kernel =
        use_int32 ? hicache_transfer_all_layer<int32_t, kElementSize, kBlockQuota, kNumThreads, kMaxOccupancy, kUsePDL>
                  : hicache_transfer_all_layer<int64_t, kElementSize, kBlockQuota, kNumThreads, kMaxOccupancy, kUsePDL>;
    LaunchKernel(num_blocks, kNumThreads, device).with_attr(kUsePDL)(kernel, params);
  }
};
