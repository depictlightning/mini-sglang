...
__global__ __launch_bounds__(kNumThreads, kMaxOccupancy) void  //
  ...
  if (warp_id < num_warps) {
    ...
    const auto src = pointer::offset(weight, pos * kSize, (warp_id % kNumSplits) * kSizePerWarp);
...
template <
    ...
    std::integral T>
__global__ __launch_bounds__(kNumThreads, kMaxOccupancy) void  //
    masked_index_kernel(const __grid_constant__ MaskedKernelParams mask_params) {
  using namespace device;
  constexpr auto kSize = kElementSize;
  constexpr auto kSizePerWarp = kSize / kNumSplits;
  constexpr auto kWarpPerBlock = static_cast<unsigned>(kNumThreads / 32);
...
struct IndexKernel {
  static void
  run(const tvm::ffi::TensorView weights,
    ...
    RuntimeCheck(entry_size == element_size, "IndexKernel: element_size mismatch.");
    ...
    if (mask_opts.has_value()) {
      ...
      const auto kernel =
          use_int32 ? masked_index_kernel<num_threads, max_concurrency, use_pdl, element_size, num_splits, int32_t>
                    : masked_index_kernel<num_threads, max_concurrency, use_pdl, element_size, num_splits, int64_t>;
      LaunchKernel(num_blocks, num_threads, device).with_attr(use_pdl)(kernel, m_params);
    } else {
      const auto kernel = use_int32
                              ? index_kernel<num_threads, max_concurrency, use_pdl, element_size, num_splits, int32_t>
                              : index_kernel<num_threads, max_concurrency, use_pdl, element_size, num_splits, int64_t>;
      LaunchKernel(num_blocks, num_threads, device).with_attr(use_pdl)(kernel, params);
    }
  }
};
