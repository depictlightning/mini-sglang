...
template <std::size_t kNumThreads, std::size_t kMaxOccupancy, bool kUsePDL, std::size_t kElementSize, std::integral T>
...
struct StoreKernel {
  ...
  run(const tvm::ffi::TensorView k_cache,
    ...
    const auto kernel = use_int32 ? store_kv_cache<num_threads, max_concurrency, use_pdl, element_size, int32_t>
                                  : store_kv_cache<num_threads, max_concurrency, use_pdl, element_size, int64_t>;
