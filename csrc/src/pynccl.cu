...
auto get_uid(const NCCLIDList& wrapper) -> ncclUniqueId {
  host::RuntimeCheck(wrapper.size() == NCCL_UNIQUE_ID_BYTES, "Invalid NCCL ID wrapper size");
  ncclUniqueId id;
...
const auto kNCCLDtypeMap = std::unordered_map<DLDataType, ncclDataType_t, DLDataTypeHash>{
...
template <typename T>
using shared_obj = shared_ptr<std::remove_pointer_t<T>>;
...
struct NCCLWrapper : public tvm::ffi::Object {
 public:
  NCCLWrapper(int rank, int world_size, const size_t max_bytes, NCCLIDList uid)
    ...
    /// NOTE: we don't deallocate here, since it may lead to unexpected program hang in exit
    ...
    ncclWindow_t win;
    NCCL_CHECK(::ncclCommWindowRegister(comm, buf, max_bytes, &win, NCCL_WIN_COLL_SYMMETRIC));
    m_win = {win, [comm = m_comm](ncclWindow_t w) { NCCL_CHECK(::ncclCommWindowDeregister(comm.get(), w)); }};
  }
  ...
  auto all_reduce(tvm::ffi::TensorView t, std::string op) const -> void {
    ...
    if (size_bytes <= m_max_bytes) {  // use internal buffer
      ...
      if (need_memcpy) {
        CUDA_CHECK(::cudaMemcpyAsync(buf_ptr, data_ptr, size_bytes, ::cudaMemcpyDeviceToDevice, stream));
      }
      NCCL_CHECK(
          ::ncclAllReduce(
              /*sendbuff=*/buf_ptr,
              ...
              /*stream=*/stream));
      if (need_memcpy) {
        CUDA_CHECK(::cudaMemcpyAsync(data_ptr, buf_ptr, size_bytes, ::cudaMemcpyDeviceToDevice, stream));
      }
    } else {
      NCCL_CHECK(
          ::ncclAllReduce(
  ...
  auto all_gather(tvm::ffi::TensorView dst, tvm::ffi::TensorView src) const -> void {
    using namespace host;
    RuntimeCheck(src.device().device_type == kDLCUDA, "Tensor must be on CUDA device");
    RuntimeCheck(src.is_contiguous(), "Tensor must be contiguous");
    RuntimeCheck(dst.device().device_type == kDLCUDA, "Tensor must be on CUDA device");
  ...
  auto get_buffer() const -> void* {
    return m_sym_mem.get();
  }

  TVM_FFI_DECLARE_OBJECT_INFO_FINAL("minisgl.NCCLWrapper", NCCLWrapper, tvm::ffi::Object);
